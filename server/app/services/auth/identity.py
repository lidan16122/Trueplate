...
from app.db.repositories import goals, profiles, users
...
async def upsert_google_user(session: AsyncSession, identity: GoogleIdentity) -> ResolvedUser:
    ...
    """
    existing = await users.google_identity(session, identity.subject)
    ...
    if existing is not None:
        ...
        if identity.email != user.email:
            ...
            clash = await users.other_email_owner(session, identity.email, user.id)
            if clash is None:
                user.email = identity.email
        if identity.picture:
            user.avatar_url = identity.picture
        await transaction.commit(session)
        await transaction.refresh(session, user)
    ...
    taken = await users.email_owner(session, identity.email)
    ...
    await users.add_user(session, user)
    await transaction.flush(session)
...
async def has_completed_onboarding(session: AsyncSession, user_id: uuid.UUID) -> bool:
    ...
    """
    profile = await profiles.profile_id(session, user_id)
    if profile is None:
        return False

    goal = await goals.active_goal_id(session, user_id)
