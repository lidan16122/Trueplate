...
There is no Redis layer; see ``services/detection/cache.py`` for why.
"""
...
from app.db.repositories import barcode_products as products
...
async def lookup(
    ...
    for candidate in candidates:
        row = await products.get(db, candidate)
        if row is not None:
    ...
    for candidate in candidates:
        ...
        await products.add_if_absent(db, row)
